...
export const GET = safeHandler<{ clientId: string }>(
  async (_, { params }): Promise<NextResponse<TResponse<User>>> => {
...
export const PUT = safeHandler<{ clientId: string }>(
  async (req, { params }): Promise<NextResponse<TResponse<BaseUser>>> => {
    const { clientId } = await params;
...
export const DELETE = safeHandler<{ clientId: string }>(
  async (_, { params }): Promise<NextResponse<TResponse>> => {
    const { clientId } = await params;
