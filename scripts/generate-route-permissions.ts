...
async function generateRoutePermissions() {
  ...
  console.log(
    `Found ${pageFiles.length} page files and ${routeFiles.length} route files`,
  );
    ...
    if (routePath === "/") {
      ...
      for (let i = 0; i <= segments.length; i++) {
        ...
        Object.keys(basePermissions).forEach((baseRoute) => {
          if (
            baseRoute.includes("[") &&
            matchesDynamicRoute(candidate, baseRoute)
          ) {
...
`;
  ...
  console.log(
    "Generated base permissions:",
    Object.keys(basePermissions).length,
  );
  console.log(
    "Generated hierarchical permissions:",
    Object.keys(hierarchicalPermissions).length,
  );
...
function matchesDynamicRoute(pathname: string, routePattern: string): boolean {
  ...
  if (
    ...
    pathSegments.every((seg) => /^[a-z0-9_-]+$/.test(seg))
  ) {
    return false;
  }
  // Catch-all
  if (routePattern.includes("[...")) {
    const beforeCatchAll = routeSegments.findIndex((seg) =>
      seg.includes("[..."),
    );
    const staticParts = routeSegments.slice(0, beforeCatchAll);
    return (
      pathSegments.length >= staticParts.length &&
      staticParts.every(
        (seg, i) => seg.includes("[") || seg === pathSegments[i],
      )
    );
  }
